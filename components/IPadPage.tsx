...
export default function IPadPage() {
  ...
  const calcScale = useCallback((orient: "landscape" | "portrait") => {
    ...
    if (orient === "landscape") {
      const labelFontSize = Math.max(
        64,
        Math.min(window.innerHeight * 0.1, 144),
      );
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const newScale = Math.max(
        0.25,
        Math.min(maxScale, resizeDragRef.current.startScale + delta),
      );
  ...
  const onIpadMouseDown = useCallback(
  ...
  const onIpadTouchStart = useCallback(
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      rotY.set(
        Math.max(
          -MAX_ROT,
          Math.min(MAX_ROT, ipadDragRef.current.ry + dx * 0.25),
        ),
      );
      rotX.set(
        Math.max(
          -MAX_ROT,
          Math.min(MAX_ROT, ipadDragRef.current.rx + dy * -0.25),
        ),
      );
    };
    const onUp = () => {
    ...
    const onSelectStart = (e: Event) => {
      if (ipadDragRef.current.active) e.preventDefault();
    };
  ...
  useEffect(() => {
    ...
    const onUp = (e: MouseEvent) => {
      if (
        Math.abs(e.clientX - startX) > 15 ||
        Math.abs(e.clientY - startY) > 15
      )
        setLocked(false);
    };
    ...
    const onTouchEnd = (e: TouchEvent) => {
      if (
        Math.abs(e.changedTouches[0].clientX - startX) > 15 ||
        Math.abs(e.changedTouches[0].clientY - startY) > 15
      )
        setLocked(false);
    };
    const onWheel = (e: WheelEvent) => {
      if (Math.abs(e.deltaX) > 10 || Math.abs(e.deltaY) > 10) setLocked(false);
    };
  ...
  const calebsPortraitStyle: React.CSSProperties = {
    background:
      "linear-gradient(175deg, #ffffff 0%, #e0eed0 50%, #9eba8e 100%)",
    backgroundClip: "text",
        ...
        {!isPortrait && (
          ...
          >
            <div
              ...
              style={{
                background:
                  "linear-gradient(160deg, #ffffff 0%, #ddeece 45%, #9eba8e 100%)",
                WebkitBackgroundClip: "text",
                backgroundClip: "text",
                color: "transparent",
              }}
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(20deg, #ffffff 0%, #d8edcc 50%, #8aab84 100%)",
                WebkitBackgroundClip: "text",
                backgroundClip: "text",
                color: "transparent",
              }}
